...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    @endif
    <cac:Signature>
        <cbc:ID>SignMamaMia</cbc:ID>
        <cbc:Note>MamaMia</cbc:Note>
        <cac:SignatoryParty>
            <cac:PartyIdentification>
        ...
        <cac:Party>
            ...
            <cac:PartyLegalEntity>
                ...
                <cac:RegistrationAddress>
                    ...
                    @endif
                    <cbc:CityName>{{ $establishment->province_description }}</cbc:CityName>
                    <cbc:CountrySubentity>{{ $establishment->department_description }}</cbc:CountrySubentity>
                    <cbc:District>{{ $establishment->district_description }}</cbc:District>
    ...
    <cac:PaymentTerms>
        ...
        <cbc:PaymentMeansID>{{ $document->method_of_payment }}</cbc:PaymentMeansID>
        @if($document->total > 0)
        <cbc:Amount currencyID="{{ $document->currency_type_id }}">{{ $document->total }}</cbc:Amount>
        @endif
    </cac:PaymentTerms>

    @if($document->payment_fees && $document->method_of_payment == 'Credito')
        @foreach($document->payment_fees as $payment_fee)
            <cac:PaymentTerms>
                <cbc:ID>FormaPago</cbc:ID>
                <cbc:PaymentMeansID>Cuota{{ str_pad($loop->iteration, 3, "0", STR_PAD_LEFT) }}</cbc:PaymentMeansID>
                <cbc:Amount currencyID="{{ $document->currency_type_id }}">{{ $payment_fee->amount }}</cbc:Amount>
                <cbc:PaymentDueDate>{{ $payment_fee->date_of_payment }}</cbc:PaymentDueDate>
            </cac:PaymentTerms>
        @endforeach
    @endif
    ...
    <cac:LegalMonetaryTotal>
        ...
        <cbc:AllowanceTotalAmount currencyID="{{ $document->currency_type_id }}">0</cbc:AllowanceTotalAmount>
        ...
        <cac:TaxTotal>
            ...
            <cac:TaxSubtotal>
                ...
                <cac:TaxCategory>
                    ...
                    @php($affectation = \App\Services\Facturacion\Templates\FunctionTribute::getByAffectation($row->affectation_igv_type_id))
                    <cac:TaxScheme>
