...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    <!-- Firma necesaria para emitir un recibo publico -->
    @if($contract)
        <cac:ContractDocumentReference>
            <cbc:ID>{{ $contract->document_supply_code }}</cbc:ID> 
            <cbc:DocumentTypeCode>{{ $contract->document_type_code }}</cbc:DocumentTypeCode>
            @if($contract->document_status_code)
            <cbc:DocumentStatusCode>{{ $contract->document_status_code }}</cbc:DocumentStatusCode>
            @endif
        </cac:ContractDocumentReference>
    @endif
    ...
    @if($delivery)
        <cac:Delivery>
            <cbc:MaximumQuantity unitCode="{{ $delivery->document_maximum_code }}">{{ $delivery->document_maximum_quantity  }}</cbc:MaximumQuantity>
            <cbc:ID schemeID='{{ $delivery->document_delivery_code }}'>{{ $delivery->document_delivery }}</cbc:ID>
            <cbc:LocationCoordinate>{{ $delivery->document_location_Coordinate  }}</cbc:LocationCoordinate>
            <cbc:Quantity/>{{ $delivery->document_quantity }}</cbc:Quantity/>
        </cac:Delivery>
    @endif
    <!-- Firma necesaria para emitir un recibo publico -->
